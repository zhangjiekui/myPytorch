...
def delete_red():
    ...
    image0 = cv2.imread(p, cv2.IMREAD_COLOR)  # 以BGR色彩读取图片
    ...
    # 显示效果
    cv2.imshow('imgs/original color image', image)
    cv2.imshow("imgs/RedThresh", RedThresh)
    cv2.imshow("imgs/erode", erode)

    # 保存图像
    cv2.imwrite('imgs/scale_image.jpg', image)
    cv2.imwrite('imgs/RedThresh.jpg', RedThresh)
    cv2.imwrite("imgs/erode.jpg", erode)
